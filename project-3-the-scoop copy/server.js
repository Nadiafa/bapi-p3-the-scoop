    ...
    'PUT': downvoteArticle
      ...
      // If body isn't supplied, user with supplied username doesn't exist, or article with supplied article ID doesn't exist, returns a 400 response
        function createComment(url, request) {
          ...
          const response = {};
          if (comment && comment.body && comment.username && database.users[comment.username] && comment.articleId && database.articles[comment.articleId]) {           
            const newComment = {
            ...
            let nextCommentId = database.nextCommentId;
            database.users[comment.username].commentIds.push(nextCommentId);
            database.comments[nextCommentId] = newComment;
            database.articles[comment.articleId].commentIds.push(nextCommentId);
            database.nextCommentId = nextCommentId + 1 ;
      ...
      // If no ID or updated comment is supplied, returns 200 response
        function updateComment(url, request) {
          ...
          if (isEmpty(request.body)) {
            response.status = 400;
          } else if (isEmpty(request.body.comment)) {
            response.status = 400;
          } else if (!database.comments[id]){
            response.status = 404;
          } else if (!request.body.comment.body){
            response.status = 404;
          } else{
      ...
      // If no ID is supplied or comment with supplied ID doesn't exist, returns 404 response
        function deleteComment(url, request) {
          ...
          if(savedComment === undefined){
            ...
            const user = database.comments[id].username; 
            const artId = savedComment.articleId; 
      
            let filterArrayofComments = database.users[user].commentIds.filter(item => item != id)
            database.users[user].commentIds = filterArrayofComments;
            
            let filterArrayofComments2 = database.articles[artId].commentIds.filter(item => item != id)
            database.articles[artId].commentIds = filterArrayofComments2;
